...
window.Map = {
	...
	init: function (blocks) {
		...
		this.blocks = blocks || [
			...
			[5, 1, 1, 1, 1, _, _, _, _, 2, 2, _, _, _, _, _, _, _, _, 6, 6, _, _, _, _, 1, 1, 1, 1, 5],
			[5,10,10,10, 1, _, _, _, _, 6, 6, _, _, _, _, _, _, _, _,14,14, _, _, _, _, 1,11,11,11, 5],
			[5,10,10,10, 1, _, _, _, _,14,14, _, _, 1,24, _, 1, _, _, 2, 2, _, _, _, _, 1,11,11,11, 5],
			[5,10,10,10, 1, _, _, _, _, 2, 2, _, _, 1, _, _, 1, _, _, 6, 6, _, _, _, _, 1,11,11,11, 5],
			[5,10,10,10, 1, _, _, _, _, 6, 6, _, _, _, _, _, _, _, _,14,14, _, _, _, _, 1,11,11,11, 5],
			[5, 1, 1, 1, 1, _, _, _, _,14,14, _, _, _, _, _, _, _, _, 2, 2, _, _, _, _, 1, 1, 1, 1, 5],
			...
			return row.map(function (col, x) {
				...
				var tile = {
					...
					type: col,
					health: 10,
					walkable: [0, 3, 4, 6, 9, 14, 20, 21, 22, 23, 24].indexOf(col) > -1,
